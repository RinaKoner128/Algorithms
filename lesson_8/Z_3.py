...
def dfs_start(g): # В результате всех прохождений статус False должен изменится на True. Это свидетельствует о корректности работы
    n = len(g)
    is_visited = [False for i in range(n)]
    print(f'До началы работы алгоритма: {is_visited}')
...
for i in range(n):
    print(g[i])
print(f'После работы алгоритма: {dfs_start(g)}')